...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        assert not self.target_critic_backup_type == "doubleq" or num_critic_networks == 2, num_critic_networks

        # TODO(student): Implement the different backup strategies.
        if self.target_critic_backup_type == "doubleq":
            next_qs = next_qs.flip(0)
        elif self.target_critic_backup_type == "min":
            next_qs = next_qs.min(0).values
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_action_entropy = next_action_entropy[None].expand((self.num_critic_networks, batch_size))
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        entropy = -torch.mean(action_distribution.log_prob(action))
        return entropy
    ...
    def update(
        ...
        if type(next_observations) == np.ndarray:
            next_observations = ptu.from_numpy(next_observations)
        if type(dones) == np.ndarray:
